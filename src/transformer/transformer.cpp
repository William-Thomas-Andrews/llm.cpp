...
Tensor Transformer::forward(int token_id, int pos) {
    ...
    return matmul(X, weights_.lm_head, LIB::BLAS, true); // logits
}
...
void Transformer::load_weights(const std::string& model_path) {
    ...
    int num_tensors;
    if (1 != fread(&num_tensors, sizeof(int), 1, f))
    ...
    for (int t = 0; t < num_tensors; t++) {
        // read name
        char name[64];
        if (64 != fread(name, 1, 64, f))
            throw std::runtime_error("Error reading file.");

        // read ndim
        int ndim;
        if (1 != fread(&ndim, sizeof(int), 1, f))
            throw std::runtime_error("Error reading file.");

        // read shape (always 8 ints in file)
        int dims[8] = {};
        if (8 != fread(dims, sizeof(int), 8, f))
            throw std::runtime_error("Error reading file.");
        ...
        if (numel != fread(tensor.data(), sizeof(float), numel, f)) 
            throw std::runtime_error("Error reading file.");
